 ...
 * role receives `false`.
 *
 * Server Actions must call this before creating a booking request. UI checks
 * may use it to hide or disable controls, but they are not authorization.
 * The helper accepts only `role` because that is the only user data needed for
 * this decision.
 */
